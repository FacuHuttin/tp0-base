...
def generate_client_content(client_id):
    ...
    name = os.getenv(f'CLIENT{client_id}_NAME', 'name')
    surname = os.getenv(f'CLIENT{client_id}_SURNAME', 'surname')
    dni = os.getenv(f'CLIENT{client_id}_DNI', '00000000')
    birthday = os.getenv(f'CLIENT{client_id}_BIRTHDAY', '0000-00-00')
    bet_number = os.getenv(f'CLIENT{client_id}_BETNUMBER', '0000')
...
if __name__ == "__main__":
  if len(sys.argv) != 3:
    print("Usage: python3 generador-compose.py <output_file> <number_of_clients>")
    sys.exit(1)
